...
class TestAuth(unittest.TestCase):
    ...
    def test_get_service_with_valid_token(self):
        ...
        ), patch("pickle.load", return_value=self.mock_creds), patch(
            "googleapiclient.discovery._retrieve_discovery_doc",
            return_value=MOCK_DISCOVERY_DOC,
        ), patch(
            "auth.build", autospec=True
        ) as mock_build:
    ...
    def test_get_service_with_expired_token(self):
        ...
        with patch("os.path.exists", return_value=True), patch("os.chmod"), patch(
            "builtins.open", mock_open()
        ), patch("pickle.load", return_value=mock_creds), patch("pickle.dump"), patch(
            "auth.Request", return_value=mock_request
        ), patch(
            "googleapiclient.discovery._retrieve_discovery_doc",
            return_value=MOCK_DISCOVERY_DOC,
        ), patch(
    ...
    def test_get_service_with_no_token(self):
        ...
        with patch("os.path.exists", return_value=False), patch("os.chmod"), patch(
            "builtins.open", mock_open()
        ), patch(
    ...
    def test_get_service_with_build_error(self):
        ...
        with patch("os.path.exists", return_value=True), patch("os.chmod"), patch(
            ...
            "auth.build", side_effect=Exception("API Error")
        ):
