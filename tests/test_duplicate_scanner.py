...
class TestDuplicateScanner(unittest.TestCase):
    ...
    def test_drive_api_get_file_metadata_error(self):
        ...
        self.mock_files_service.get.return_value.execute.side_effect = Exception(
            "API Error"
        )
    ...
    def test_drive_api_list_files_error(self):
        ...
        self.mock_files_service.list.return_value.execute.side_effect = Exception(
            "API Error"
        )
    ...
    def test_duplicate_scanner(self):
        ...
        with patch.object(
            self.drive_api, "get_file_metadata", side_effect=mock_get_metadata
        ), patch.object(
            self.drive_api,
            "get_files_metadata_batch",
            side_effect=mock_get_metadata_batch,
        ):
            ...
            self.assertEqual(
                len(scanner.duplicate_groups), 1
            )  # One group of duplicates
            self.assertEqual(
                len(scanner.duplicate_groups[0].files), 2
            )  # Two files in the group
    ...
    def test_batch_handler_operations(self):
        ...
        for file_id in file_ids[:2]:  # First two succeed
            callback = mock_batch.add.call_args_list[file_ids.index(file_id)][1][
                "callback"
            ]
    ...
    def test_drive_api_batch_operations(self):
        ...
        with patch.object(
            DriveAPI, "_get_batch_handler", return_value=mock_handler
    ...
    def test_drive_api_batch_size_limits(self):
        ...
        mock_batch = MagicMock()
        mock_batch.execute.return_value = None
        mock_batch.get_results.return_value = {
            f"id{i}": {"id": f"id{i}"} for i in range(BATCH_SIZE)
        }
    ...
    def test_main_script_flow(self):
        ...
        with patch("sys.argv", ["duplicate_scanner.py"]), patch(
            "duplicate_scanner.get_service", return_value=mock_service
        ), patch("duplicate_scanner.DriveAPI", mock_drive_api), patch(
            "duplicate_scanner.MetadataCache", return_value=mock_cache
        ), patch(
            "duplicate_scanner.write_to_csv"
        ) as mock_write_csv:
    ...
    def test_write_to_csv_optimized(self, mock_tqdm):
        ...
        mock_tqdm.assert_called_once_with(
            total=3, desc="Exporting duplicates", unit="files"
        )
        self.assertEqual(
            mock_progress.update.call_count, 3
    ...
    def test_write_to_csv_with_missing_metadata(self, mock_tqdm):
        ...
        # Verify progress bar was used correctly
        mock_tqdm.assert_called_once_with(
            total=2, desc="Exporting duplicates", unit="files"
        )
        self.assertEqual(
            mock_progress.update.call_count, 2
        )  # Called for both files, even the missing one
        ...
        with open(filename, "r") as f:
            ...
            self.assertEqual(rows[0]["File Name"], "file1.txt")
            self.assertEqual(
                rows[0]["Duplicate File Name"], ""
            )  # No duplicates due to missing metadata
    ...
    def test_write_to_csv_with_empty_groups(self, mock_tqdm):
        ...
        # Verify progress bar was used correctly
        mock_tqdm.assert_called_once_with(
            total=0, desc="Exporting duplicates", unit="files"
        )
        self.assertEqual(
            mock_progress.update.call_count, 0
        )  # Never called for empty group
    ...
    def test_drive_api_batch_size_logging(self):
        ...
        for file_ids, expected_batches in test_cases:
            ...
            mock_batch.execute.return_value = None
            mock_batch.get_results.return_value = {
                f"id{i}": {"id": f"id{i}"} for i in range(len(file_ids))
            }
            mock_batch.get_failed_requests.return_value = set()

            with patch("src.drive_api.BatchHandler", return_value=mock_batch), patch(
                "logging.info"
            ) as mock_logging:
