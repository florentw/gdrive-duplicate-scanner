...
from src.scanner import (
    BaseDuplicateScanner,
    DuplicateScanner,
    DuplicateScannerWithFolders,
)
...
class TestDuplicateScannerWithFolders(unittest.TestCase):
    ...
    def test_scan_with_cache(self):
        ...
        test_folders = [{"id": "folder1", "name": "Test Folder"}]
        self.cache.get_all_files.return_value = test_files
    ...
    def test_scan_without_cache(self):
        ...
        self.drive_api.list_all_files_and_folders.return_value = (
            test_files,
            test_folders,
        )
        self.drive_api.list_files.return_value = test_files
    ...
    def test_duplicate_only_folders(self):
        ...
        # Verify results
        self.assertEqual(
            len(self.scanner.duplicate_groups), 2
        )  # Two groups of duplicates
        self.assertEqual(
            len(self.scanner.duplicate_files_in_folders), 2
        )  # Both folders have duplicates
        self.assertEqual(
            len(self.scanner.duplicate_only_folders), 2
        )  # Both folders contain only duplicates
    ...
    def test_duplicate_only_folders_mixed_content(self):
        ...
        self.assertEqual(
            len(self.scanner.duplicate_only_folders), 1
        )  # Only folder2 contains only duplicates
