...
class BaseDuplicateScanner:
    ...
    def _filter_valid_files(self, files: List[Dict]) -> List[Dict]:
        ...
        return [
            ...
            and not file.get("mimeType", "").startswith(
    ...
    def _process_duplicate_group(
        self, files: List[Dict], metadata: Dict[str, dict]
    ) -> None:
    ...
    def _scan_for_duplicates(self, files: List[Dict]) -> None:
        ...
        self.logger.info(
            f"Found {len(valid_files)} valid files to check for duplicates"
        )
        ...
        with tqdm(
            total=len(size_groups), desc="Scanning for duplicates", unit="size group"
        ) as pbar:
            for size, files in size_groups.items():
                if (
                    len(files) > 1
                ):  # Only check if there are multiple files of the same size
...
class DuplicateScannerWithFolders(BaseDuplicateScanner):
    ...
    def scan(self) -> None:
        ...
        print(f"Total wasted space: {wasted_gb:.2f} GB")
        print(
            f"Found {len(self.duplicate_files_in_folders)} folders with duplicate files"
        )
        print(
            f"Found {len(self.duplicate_only_folders)} folders containing only duplicate files"
        )
        ...
        with tqdm(total=len(folders), desc="Analyzing folders", unit="folder") as pbar:
            for folder in folders:
                ...
                if folder_id in folder_files:
                    ...
                    if duplicate_files:
                        folder_obj = DuplicateFolder(folder_id, folder, duplicate_files)
                        folder_obj.total_files = folder_total_files.get(
                            folder_id, set()
                        )
