...
class DriveAPI:
    ...
    def get_file_metadata(self, file_id: str) -> Optional[dict]:
        ...
        try:
            ...
            file = (
                self.service.files()
                .get(fileId=file_id, fields=METADATA_FIELDS)
                .execute()
            )
    ...
    def _handle_failed_requests(
        self, failed_ids: set[str], results: Dict[str, dict]
    ) -> None:
    ...
    def get_files_metadata_batch(self, file_ids: list[str]) -> Dict[str, dict]:
        ...
        logging.info(
            f"Processing {total_files} files in {total_batches} batches (avg {avg_batch_size:.1f} files per batch, {self.api_request_count} API requests so far)"
        )
